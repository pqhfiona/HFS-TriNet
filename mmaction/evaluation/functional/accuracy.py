...
def positive_accuracy(preds, labels):
    ...
    correct_positives = sum(1 for i in positive_indices if np.argmax(preds[i]) == 1)
    print("correct_positives:", correct_positives)
    return correct_positives / len(positive_indices) if positive_indices else 0


def negative_accuracy(preds, labels):
    negative_indices = [i for i, label in enumerate(labels) if label == 0]
    correct_negatives = sum(1 for i in negative_indices if np.argmax(preds[i]) == 0)
    print("correct_negatives:", correct_negatives)
    return correct_negatives / len(negative_indices) if negative_indices else 0


def sensitivity(preds, labels):
    TP = sum(1 for pred, label in zip(preds, labels) if label == 1 and np.argmax(pred) == 1)
    FN = sum(1 for pred, label in zip(preds, labels) if label == 1 and np.argmax(pred) == 0)
    return TP / (TP + FN) if (TP + FN) != 0 else 0


def f1_score(preds, labels):
    TP = sum(1 for pred, label in zip(preds, labels) if label == 1 and np.argmax(pred) == 1)
    FP = sum(1 for pred, label in zip(preds, labels) if label == 0 and np.argmax(pred) == 1)
    FN = sum(1 for pred, label in zip(preds, labels) if label == 1 and np.argmax(pred) == 0)
...
def AUC(preds, labels):
    def roc_curve_internal(preds, labels):
        thresholds = sorted(list(set([p[1] for p in preds])), reverse=True)

        tpr = []
        fpr = []

        for threshold in thresholds:
            TP = sum(1 for pred, label in zip(preds, labels) if label == 1 and pred[1] >= threshold)
            FP = sum(1 for pred, label in zip(preds, labels) if label == 0 and pred[1] >= threshold)
            FN = sum(1 for pred, label in zip(preds, labels) if label == 1 and pred[1] < threshold)
            TN = sum(1 for pred, label in zip(preds, labels) if label == 0 and pred[1] < threshold)
    ...
    fpr, tpr = roc_curve_internal(preds, labels)
...
def save_video_scores_to_csv(frame_dirs, labels, preds, save_path='video_scores.csv'):
    os.makedirs(os.path.dirname(save_path), exist_ok=True)

    with open(save_path, 'w', newline='') as f:
        writer = csv.writer(f)
        num_classes = len(preds[0])
        header = ['frame_dir', 'label'] + [f'score_{i}' for i in range(num_classes)]
        writer.writerow(header)

        for frame_dir, label, pred in zip(frame_dirs, labels, preds):
            row = [frame_dir, label] + list(map(float, pred))
            writer.writerow(row)