...
class Recognizer3D(BaseRecognizer):
    ...
    def extract_feat(self,
        ...
        wt_features_stacked = None
        if data_samples:
            wt_features = [sample.get('wt_features') for sample in data_samples]
            if all(f is not None for f in wt_features):
                wt_features_stacked = torch.stack(wt_features, dim=0) \
                    .permute(1, 4, 0, 2, 3) \
                    .float()
        ...
        if test_mode:
            ...
            x = self.backbone(inputs)
            if wt_features_stacked is not None:
                y = self.backbone(wt_features_stacked)
            else:
            ...
            loss_aux = dict()
            if self.with_neck:
                x, loss_aux = self.neck(x, y, data_samples=data_samples)
