...
class FocalLoss(BaseWeightedLoss):
    ...
    Focal Loss is designed to address the issue of class imbalance by down-weighting
    ...
    Args:
        ...
        gamma (float): Focusing parameter for modulating the loss. Defaults to 2.0.
    """
    ...
    def _forward(self, cls_score: torch.Tensor, label: torch.Tensor,
        ...
        if cls_score.size() == label.size():
            ...
            if self.class_weight is not None:
                ...
                lsm = lsm * self.class_weight.unsqueeze(0)
            pt = torch.exp(lsm)
            loss_cls = -self.alpha * (1 - pt) ** self.gamma * label * lsm
            ...
            ce_loss = F.cross_entropy(cls_score, label, reduction='none', **kwargs)
            pt = torch.exp(-ce_loss)
            loss_cls = self.alpha * (1 - pt) ** self.gamma * ce_loss
