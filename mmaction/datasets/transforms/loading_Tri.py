...
class SampleFrames(BaseTransform):
    ...
    def _get_train_clips(self, num_frames: int,
        ...
        if self.keep_tail_frames:
            ...
            if avg_interval > 1:
                ...
                rand_ints = np.random.randint(avg_interval, size=self.num_clips)
                clip_offsets = base_offsets + rand_ints


            elif num_frames > max(self.num_clips, ori_clip_len):
                rand_ints = np.random.randint(num_frames - ori_clip_len + 1, size=self.num_clips)
                clip_offsets = np.sort(rand_ints)
    ...
    def transform(self, results: dict) -> dict:
        ...
        frame_inds = np.array([int(idx) for idx in frame_inds], dtype=np.int32)
        frame_dir = results['frame_dir'][-8:]
        feature_dir = f'/data/dataset/US_1083/all_NPZ/{frame_dir}/'
        wt_dir = f'/data/dataset/US_1083/all_wtconv/{frame_dir}/'

        features = []
        for idx in frame_inds:
            npz_filename = f'img_{str(idx).zfill(5)}.npz'
            feature_path = os.path.join(feature_dir, npz_filename)

            try:
                with np.load(feature_path, allow_pickle=True) as feature_file:
                    features.append(feature_file['img_embeddings'])
                    if feature_file['img_embeddings'].shape != (256, 64, 64):
                        print(f"Unexpected feature shape for {npz_filename}: {feature_file['img_embeddings'].shape}")
            except FileNotFoundError:
                print(f"Feature file for {frame_dir}({npz_filename}) in {feature_dir} not found.")
                features.append(np.zeros((256, 64, 64)))
            except Exception as e:
                print(f"An error occurred while loading {frame_dir}({npz_filename}): {e}")
                features.append(np.zeros((256, 64, 64)))
        features = np.array(features)

        wt_features = []
        for idx in frame_inds:
            img_filename = f'img_{str(idx).zfill(5)}.jpg'
            img_path = os.path.join(wt_dir, img_filename)

            try:
                img = Image.open(img_path)
                img = img.convert('RGB')
                img_array = np.array(img)
                wt_features.append(img_array)
            except FileNotFoundError:
                print(f"Image file for {frame_dir}({img_filename}) in {wt_dir} not found.")
                wt_features.append(np.zeros((256, 256, 3)))
            except Exception as e:
                print(f"An error occurred while loading {frame_dir}({img_filename}): {e}")
                wt_features.append(np.zeros((256, 256, 3)))
        wt_features = np.array(wt_features)


        results['wt_features'] = torch.tensor(wt_features)
        results['features'] = torch.tensor(features)
...
class RawFrameDecode(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        # print(f"Keys in results1: {results.keys()}")
        return results
